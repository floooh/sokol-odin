...
when ODIN_OS == .Windows {
    ...
    when ODIN_DEBUG == true { foreign import sokol_gl_clib { "sokol_gl_linux_x64_gl_debug.lib" } }
    else                    { foreign import sokol_gl_clib { "sokol_gl_linux_x64_gl_release.lib" } }
}
...
foreign sokol_gl_clib {
    ...
    sgl_defaults :: proc()  ---
    sgl_viewport :: proc(x: i32, y: i32, w: i32, h: i32, origin_top_left: b8)  ---
    sgl_viewportf :: proc(x: f32, y: f32, w: f32, h: f32, origin_top_left: b8)  ---
    sgl_scissor_rect :: proc(x: i32, y: i32, w: i32, h: i32, origin_top_left: b8)  ---
    sgl_scissor_rectf :: proc(x: f32, y: f32, w: f32, h: f32, origin_top_left: b8)  ---
...
viewport :: proc(x: int, y: int, w: int, h: int, origin_top_left: bool)  {
    sgl_viewport(cast(i32)x, cast(i32)y, cast(i32)w, cast(i32)h, cast(b8)origin_top_left);
}
viewportf :: proc(x: f32, y: f32, w: f32, h: f32, origin_top_left: bool)  {
    sgl_viewportf(x, y, w, h, cast(b8)origin_top_left);
}
scissor_rect :: proc(x: int, y: int, w: int, h: int, origin_top_left: bool)  {
    sgl_scissor_rect(cast(i32)x, cast(i32)y, cast(i32)w, cast(i32)h, cast(b8)origin_top_left);
}
scissor_rectf :: proc(x: f32, y: f32, w: f32, h: f32, origin_top_left: bool)  {
    sgl_scissor_rectf(x, y, w, h, cast(b8)origin_top_left);
}
enable_texture :: proc()  {
    sgl_enable_texture();
}
